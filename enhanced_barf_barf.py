...
class Model(nerf.Model):
    ...
    def __init__(self,opt):
        super().__init__(opt)
        # 적응형 스케줄러를 위한 커스텀 변수 추가
        self.adaptive_progress = 0.0
    ...
    def train_iteration(self,opt,var,loader):
        ...
        # ==========================================
        # 적응형 스케줄러 (Adaptive Scheduler) 적용
        # ==========================================
        current_loss = loss.all.item()

        # 1. 오차가 0.08 미만으로 떨어지면 카운트 증가 
        if current_loss < 0.08:
            self.patience_counter += 1
        else:
            self.patience_counter = 0 # 오차가 다시 튀면 카운트 초기화

        # 2. 50번 연속으로 오차가 낮게 유지되면 진짜 수렴했다고 판단하고 진행률 증가
        if self.patience_counter > 50:
            self.adaptive_progress = min(1.0, self.adaptive_progress + 0.001)

        # 3. adaptive_progress을 모델에 전달
        self.graph.nerf.progress.data.fill_(self.adaptive_progress)
        if opt.nerf.fine_sampling:
            self.graph.nerf_fine.progress.data.fill_(self.adaptive_progress)
