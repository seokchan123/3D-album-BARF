...
class Graph(base.Graph):
    ...
    def compute_loss(self,opt,var,mode=None):
        ...
        # ==========================================
        if mode == "train":
            opt.loss_weight.setdefault("sparsity", -3.0)
            if "weight" in var:
                loss.sparsity = torch.mean(var.weight ** 2)
            if opt.nerf.fine_sampling:
                opt.loss_weight.setdefault("sparsity_fine", -3.0)
                if "weight_fine" in var:
                    loss.sparsity_fine = torch.mean(var.weight_fine ** 2)

        # ==========================================
        # 단일 이미지 깊이(Depth) 가이드 결합
        # ==========================================
        if mode == "train" and "depth_gt" in var:
            depth_gt = var.depth_gt.view(batch_size, opt.H*opt.W)
            if opt.nerf.rand_rays:
                depth_gt = depth_gt[:, var.ray_idx]
            
            # 정답지의 차원을 모델 예측값(var.depth)과 강제로 똑같이 맞춤
            depth_gt = depth_gt.view_as(var.depth)
            
            # 스케일 불변(Scale-Invariant) 정규화
            depth_gt_norm = (depth_gt - depth_gt.min()) / (depth_gt.max() - depth_gt.min() + 1e-8)
            depth_pred_norm = (var.depth - var.depth.min()) / (var.depth.max() - var.depth.min() + 1e-8)
            
            opt.loss_weight.setdefault("depth", -2.0)
            loss.depth = self.MSE_loss(depth_pred_norm, depth_gt_norm) 
            
            if opt.nerf.fine_sampling and "depth_fine" in var:
                opt.loss_weight.setdefault("depth_fine", -2.0)
                depth_gt_fine = depth_gt.view_as(var.depth_fine)
                
                # 정밀(Fine) 모델에도 동일하게 정규화 적용
                depth_gt_fine_norm = (depth_gt_fine - depth_gt_fine.min()) / (depth_gt_fine.max() - depth_gt_fine.min() + 1e-8)
                depth_pred_fine_norm = (var.depth_fine - var.depth_fine.min()) / (var.depth_fine.max() - var.depth_fine.min() + 1e-8)
                
                loss.depth_fine = self.MSE_loss(depth_pred_fine_norm, depth_gt_fine_norm)
